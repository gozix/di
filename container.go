...
func (c *container) resolve(tv *reflect.Value, cycle *cycle.Cycle, modifiers []Modifier) (err error) {
	if tv.Kind() != reflect.Pointer && tv.Kind() != reflect.Slice {
		return fmt.Errorf("target %w, got %s", ErrMustBeSliceOrPointer, tv.Kind())
	}
	...
	if len(defs) == 0 {
		...
		if len(defs) == 0 {
			return fmt.Errorf("type %s %w", tv.Type(), ErrDoesNotExist)
		}
	}

	if ft.Kind() != reflect.Slice && len(defs) > 1 {
		return fmt.Errorf("%w of %s", ErrMultipleDefinitions, tv.Type())
	}

	for _, def := range defs {
		if err = cycle.Add(def.id); err != nil {
			return fmt.Errorf("unable resolve %s : %w", tv.Type(), err)
		}
		...
		if err != nil {
			cycle.Del(def.id)
			return fmt.Errorf("unable create value : %w", err)
		}
